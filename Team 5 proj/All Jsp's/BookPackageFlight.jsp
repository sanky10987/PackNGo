...
try {		
			  ...
			  numAdult = Integer.parseInt(session.getAttribute("numAS").toString());
			  numChild = Integer.parseInt(session.getAttribute("numCS").toString());
			  ...
			  while (rs.next())
				...
				%>
				  ...
				  <form action="PaymentPckFlightForm.jsp" method="post"> 
				  <table border="2" cellpadding="5" cellspacing="3" BORDERCOLOR="#151B54">
				  <thead><h2><b>Your  Booking  Selection</b></h2></thead>
				  <tr>
				  <td><b>Flight Name</b></td><td><b>Source</b></td><td><b>Destination</b></td><td><b>Departure On</b></td><td><b>No. of Adults</b></td><td><b>No. of Children</b></td><td><b>Total Cost</b></td>
				  </tr>
				  <tr>
				  <td><%=rs.getString("flight_name")%></td><td><%=rs.getString("source")%></td><td><%=rs.getString("Destination")%></td><td><%=rs.getString("Date_of_flight").toString().substring(0,11)%></td><td><%=session.getAttribute("numAS").toString()%></td><td><%=session.getAttribute("numCS").toString()%></td><td><%=totalCost%></td>
				  </tr>
...
<jsp:setProperty name="OBean" property="packageId" value="<%=Integer.parseInt(session.getAttribute("packageId").toString())%>" />
...
<jsp:setProperty name="OBean" property="startDate" value="<%=session.getAttribute("packageDate").toString()%>" />
